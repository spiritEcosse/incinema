...
def get_detailed_audio_properties(file_path):
    ...
    try:
        ...
        # For PCM, we need bits_per_sample
        bits_per_sample = int(stream.get('bits_per_sample', 0))
        if bits_per_sample > 0:
            # For PCM formats
            properties['sample_format'] = 1
        else:
            ...
            compressed_formats = ['mp3', 'aac', 'opus', 'vorbis', 'flac', 'alac', 'wmav2']
            if any(fmt in codec_name for fmt in compressed_formats):
                properties['sample_format'] = 8
            else:
                raise ValueError(f"Cannot determine sample format for codec: {codec_name}")

        # Map codec name to Movavi codec ID (required)
        if 'codec_name' not in stream:
        ...
        properties['codec_id'] = codec_map.get(codec_name, f"CODEC_ID_{codec_name.upper()}")
    ...
    try:
        ...
        try:
            ...
            if "duration" in video_stream:
                ...
                frame_rate = metadata["frame_rate_n"] / metadata["frame_rate_d"]
                metadata["duration_frames"] = int(duration_sec * frame_rate)
            elif "nb_frames" in video_stream:
                metadata["duration_frames"] = int(video_stream["nb_frames"])
            else:
        ...
        if audio_stream:
            try:
                ...
                codec_id = audio_stream.get("codec_name")

                if not all([sample_rate, channel_layout, sample_format, codec_id]):
                    missing = []
                    ...
                    if not codec_id: missing.append("codec_id")
